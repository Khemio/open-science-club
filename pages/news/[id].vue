...
const { data: announcement } = await useAsyncData('announcement', async () => {
...
<template>
    ...
    <ul v-if="announcement[0].news_comments" class="w-11/12 mb-3 mx-auto flex flex-col gap-3">
        <li v-for="comment in announcement[0].news_comments" class="min-w-full px-3 py-1 border border-black">
